...
- `manifest/release-differences.csv`: changed estimates relative to v0.1.1.
- `generated/manuscript/methods-results.md`: recreated by `code/09_manuscript.mjs` after every run.
...
**Primary** identifies the first pipeline stage, not confirmatory status: reproduction of the Cochrane class pools followed by investigator-defined restrictions motivated by the published response, raw-scale mean differences, and continuous amyloid-clearance meta-regression. These analyses were not prospectively registered.
...
| `tables/cochrane_compatible_class_pool_results.csv` | Class-pool estimates using each Cochrane analysis's recorded CI method and Cochrane-compatible p-value convention |
| `tables/cochrane_compatible_reproduction_check.csv` | Direct confidence-interval and p-value comparison for the Cochrane-compatible class pools |
...
For direct comparison with Cochrane, the primary script also writes a separately labelled Cochrane-compatible class-pool output. It uses the `CI method` recorded in `CD016297-overall-estimates-and-settings.csv` for each analysis (15 HKSJ and 26 Wald settings among groups 1–9) and uses the exported Effect P convention, which closely matches normal/Wald p-values even when the CI method is HKSJ. The main sensitivity tables retain the internally consistent REML/HKSJ convention described above.
...
The Cochrane-compatible comparison is provided separately so that exact review-oriented checks do not silently change the inferential convention used by the response-driven sensitivity analyses.
