...
results/
  ...
  primary/figures/                Primary statistical figures
  extended/tables/                Published extended CSV and JSON snapshots
  extended/figures/               Extended statistical figures
  reports/                        Published PDF and Excel presentation files
site/
...
All primary snapshots are under `results/primary/` and are generated by `code/01_primary_reanalysis.R`.
...
All extended snapshots are under `results/extended/` and are generated by `code/02_extended_reanalysis.R` from the newly generated primary rows.
...
`results/reports/` contains the published primary and extended PDF reports and Excel workbooks. They are presentation snapshots derived from the statistical JSON and figures. They do not feed back into the analyses or the website. The one-command pipeline intentionally treats them as optional: reproducing the statistical tables and website registry does not require the specialized workbook/report formatting runtime.
